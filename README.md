...
Pilot scope: X7R ceramic capacitors, leakage current in µA. All data are synthetic; the results show the pipeline works, not field accuracy. The release forecaster `xgboost_v2` reaches normalized MAE 0.136 on untouched test batches against 0.153 for persistence.
...
The full suite (273 tests), then the same CSV through the HTTP API and the core directly, failing on any disagreement.
