...
MIN_CHANNEL_PEERS = 5
CHANNEL_PEER_Z_THRESHOLD = 3.0
SLOPE_Z_STRATA_EDGES = (2.0, 5.0)  # interval strata on signed slope_batch_robust_z: (-inf,2), [2,5), [5,inf)
...
SOURCE_FILES_FOR_PROVENANCE = ("forecast_v2.py", "features.py", "mlcc_prototype.py", "validation.py")


def provenance_record() -> dict[str, Any]:
    """Everything outside the configuration list that changes results: shared
    hyperparameters, feature-formula constants and the source files themselves."""
    here = Path(__file__).resolve().parent
    return {
        "v1_xgb_params": dict(V1_XGB_PARAMS),
        "feature_constants": {"MIN_CHANNEL_PEERS": MIN_CHANNEL_PEERS, "CHANNEL_PEER_Z_THRESHOLD": CHANNEL_PEER_Z_THRESHOLD},
        "v2_feature_formulas": dict(V2_FEATURE_FORMULAS),
        "interval_constants": {"SLOPE_Z_STRATA_EDGES": list(SLOPE_Z_STRATA_EDGES), "MIN_STRATUM_CALIBRATION": MIN_STRATUM_CALIBRATION},
        "source_sha256": {name: sha256((here / name).read_bytes()).hexdigest() for name in SOURCE_FILES_FOR_PROVENANCE if (here / name).exists()},
    }


def provenance_digest(record: dict[str, Any] | None = None) -> str:
    """Digest over the hyperparameters and constants (not the source hashes, which
    legitimately change with documentation edits; those are compared separately)."""
    record = record or provenance_record()
    payload = json.dumps({k: record[k] for k in ("v1_xgb_params", "feature_constants", "v2_feature_formulas", "interval_constants")}, sort_keys=True).encode()
    return sha256(payload).hexdigest()
...
def load_training_data(data_dir: str | Path) -> TrainingData:
    ...
    for path in (early_path, label_path):
        if not path.exists():
            raise FileNotFoundError(f"{path} not found; --data-dir must point at the folder holding train_early.csv and train_labels.csv (the pack uses data/, the release workspace outputs/mlcc_v1/)")
...
def augment_features(features: pd.DataFrame, early: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    for _, index in keys.loc[valid].groupby(peer_keys + ["channel"], sort=False).indices.items():
        ...
        for position, row in enumerate(index):
            ...
            elevated_fraction[row] = float(np.mean(peers > CHANNEL_PEER_Z_THRESHOLD))
...
# --------------------------------------------------------------------------- #
def choose_calibration_batches(train_features: pd.DataFrame, *, per_profile: int, rotation: int = 0) -> list[str]:
    """Deterministically reserve ``per_profile`` batches of every profile inside a
    fold's training portion for calibration. ``rotation`` (the fold index) shifts
    the window through the profile's sorted batch ids so different folds
    calibrate on different batches instead of always the highest ids."""
    ...
    for _, group in batches.sort_values(["profile_id", "batch_id"]).groupby("profile_id", sort=True):
        ...
        start = (rotation * per_profile) % len(ids)
        chosen.extend(ids[(start + j) % len(ids)] for j in range(per_profile))
...
def run_interval_experiment(
    ...
    *,
    ...
    for k, train_mask, valid_mask in fold_masks(features, folds):
        ...
        cal_batches = choose_calibration_batches(train_features, per_profile=calibration_batches_per_profile, rotation=k)
    ...
    return {
        "config": config.to_dict(), "alpha": alpha, "calibration_rule": f"{calibration_batches_per_profile} whole batch(es) per profile inside each fold's training portion, window rotated by fold index through the profile's sorted batch ids; validation residuals never calibrate their own interval",
...
WEAK_PEER_THRESHOLD = 8  # v1's weak-peer rule
OUTPUT_COLUMNS = IDENTITY + [
    "status", "predicted_final_value", "predicted_normalized", "unit", "target_hour", "as_of_hour",
    "upper_limit", "value_at_24h", "persistence_final_value", "profile_id", "peer_count", "weak_peer_warning",
    "imputed_feature_count", "unavailable_reason", "candidate_version", "candidate_name",
]
MANIFEST_DIGEST_ATTRIBUTE = "claude_forecast_v2_manifest_sha256"


MANIFEST_DIGEST_EXCLUDED = ("artifact_sha256", "environment")


def manifest_digest(manifest: dict[str, Any]) -> str:
    """Digest of the behaviour-determining manifest content. Excludes the artifact
    hashes (computed after the model files, whose attributes carry this digest)
    and the informational environment block (the version check governs that)."""
    canonical = {k: v for k, v in manifest.items() if k not in MANIFEST_DIGEST_EXCLUDED}
    return sha256(json.dumps(_json_safe(canonical), sort_keys=True).encode()).hexdigest()


def validate_manifest(manifest: dict[str, Any]) -> CandidateConfig:
    """Schema checks so a tampered or incomplete manifest fails at load, not at first predict."""
    for key in ("candidate_version", "config", "feature_columns", "imputation_medians", "target", "supported_profiles", "environment"):
        if key not in manifest:
            raise ValueError(f"manifest is missing {key}")
    config_fields = dict(manifest["config"]); config_fields.pop("feature_columns", None)
    try:
        config = CandidateConfig(**config_fields)
    except (TypeError, ValueError) as error:
        raise ValueError(f"manifest config is invalid: {error}") from error
    if list(manifest["feature_columns"]) != config.feature_columns:
        raise ValueError("manifest feature_columns disagree with the configuration's feature set")
    if not set(config.feature_columns).issubset(manifest["imputation_medians"]):
        raise ValueError("manifest imputation_medians do not cover every feature column")
    if manifest["target"].get("parameterisation") != config.target:
        raise ValueError("manifest target parameterisation disagrees with the configuration")
    if bool(manifest["target"].get("base_margin_required_at_predict")) != (config.model == "xgboost" and config.target.endswith("residual_over_persistence")):
        raise ValueError("manifest base_margin flag disagrees with the configuration")
    if not isinstance(manifest["supported_profiles"], list):
        raise ValueError("manifest supported_profiles must be a list")
    return config
...
class ForecastCandidate:
    ...
    def predict_records(self, early_readings: pd.DataFrame) -> list[dict[str, Any]]:
        """``predict`` as JSON-safe dictionaries (NaN/NA become null); convenient for an API layer."""
        frame = self.predict(early_readings)
        return [_json_safe({k: (None if pd.isna(v) else v) for k, v in row.items()}) for row in frame.to_dict("records")]
    ...
    def predict(self, early_readings: pd.DataFrame) -> pd.DataFrame:
        ...
        if not features.empty:
            ...
            if not scored.empty:
                ...
                config = self.config
                imputed = feature_matrix(scored, config.feature_columns).isna().sum(axis=1).to_numpy() if config.model == "xgboost" else np.zeros(len(scored), dtype=int)
                ...
                for index, row in scored.iterrows():
                    rows.append({
                        **{c: row[c] for c in IDENTITY},
                        ...
                        "weak_peer_warning": bool(peer_counts[index] < WEAK_PEER_THRESHOLD),
                        "imputed_feature_count": int(imputed[index]),
                        "unavailable_reason": None,
                        "candidate_version": VERSION, "candidate_name": self.manifest["config"]["name"],
                    })
        out = pd.DataFrame(rows, columns=OUTPUT_COLUMNS).sort_values(SORT_KEYS).reset_index(drop=True)
        # Stable dtypes whether or not the frame holds unavailable rows.
        for column in ("predicted_final_value", "predicted_normalized", "upper_limit", "value_at_24h", "persistence_final_value"):
            out[column] = pd.to_numeric(out[column], errors="coerce").astype(float)
        for column in ("peer_count", "imputed_feature_count"):
            out[column] = out[column].astype("Int64")
        out["weak_peer_warning"] = out["weak_peer_warning"].astype("boolean")
        for column in ("profile_id", "unavailable_reason"):
    ...
    def _unavailable_row(self, identity: dict[str, Any], reason: str) -> dict[str, Any]:
        return {
            **identity, "status": UNAVAILABLE_STATUS, "predicted_final_value": None, "predicted_normalized": None,
            ...
            "weak_peer_warning": None, "imputed_feature_count": None,
    ...
    *,
    ...
    manifest = {
        ...
        "assumptions": [
            ...
            "The batch robust-z features together with the raw fractions implicitly encode the uploaded batch's median and scale of 24 h leakage and slope (v1 design); whole-batch holdout is the honest test of that dependence",
            "acceleration_fraction_per_hour2 is identically zero with two checkpoints and acts as a constant placeholder",
...
def save_forecast_candidate(candidate: ForecastCandidate, path: str | Path) -> Path:
    """Write manifest.json plus native model.ubj / model.json. The manifest's
    digest is embedded in the model files as a booster attribute and the model
    files' hashes are written into the manifest, so neither can be edited alone."""
    directory = Path(path)
    directory.mkdir(parents=True, exist_ok=True)
    manifest = {k: v for k, v in candidate.manifest.items() if k != "artifact_sha256"}
    manifest["manifest_sha256_binding"] = f"booster attribute {MANIFEST_DIGEST_ATTRIBUTE}" if candidate.booster is not None else "none (baseline candidate has no model file)"
    artifacts: dict[str, str] = {}
    if candidate.booster is not None:
        candidate.booster.get_booster().set_attr(**{MANIFEST_DIGEST_ATTRIBUTE: manifest_digest(manifest)})
...
def load_forecast_candidate(path: str | Path, *, strict_versions: bool = True) -> ForecastCandidate:
    ...
    if config.model == "xgboost" and set(manifest.get("artifact_sha256", {})) != {"model.ubj", "model.json"}:
        raise ValueError("manifest artifact_sha256 must list exactly model.ubj and model.json")
    for name, digest in manifest.get("artifact_sha256", {}).items():
        if not (directory / name).exists() or _hash_file(directory / name) != digest:
    ...
    if mismatched:
        ...
        warnings.warn(f"Loading candidate with mismatched library versions {mismatched}; validation results do not transfer", RuntimeWarning, stacklevel=2)
    booster = None
    if config.model == "xgboost":
        ...
        bound = booster.get_booster().attr(MANIFEST_DIGEST_ATTRIBUTE)
        if bound != manifest_digest(manifest):
            raise ValueError("Manifest integrity check failed: manifest.json does not match the digest bound into the model files")
