...
FastAPI service that validates an uploaded burn-in CSV, runs the frozen MLCC bundle once per request, and returns a typed JSON response the dashboard pins against. Models load once at startup; a request only runs inference.
...
- [STATUS.md](STATUS.md): what has been verified and the known limitations
